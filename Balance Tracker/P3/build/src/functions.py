...
def normalize_textbox_values(txn_name: str, txn_amount: str, txn_date: str) -> tuple[str, float, datetime]:
    ...
    if txn_name != name_placeholder:
        ...
        if txn_name != "":
            #print("'txn_name correctly recovered in normalize_textbox_values()")
    ...
    if txn_amount != amount_placeholder:
        ...
        if txn_amount != "":
            #print("'txn_amount correctly recovered in normalize_textbox_values()")
    ...
    if txn_date != date_placeholder:
        ...
        if txn_date != "":
            try:
                #print("'txn_date correctly recovered in normalize_textbox_values()")
                txn_date: datetime = datetime.strptime(txn_date, "%Y-%m-%d %H:%M")
                txn_valid_data[2] = txn_date

            except ValueError:
                print("'txn_date is not '%Y-%m-%d %H:%M', trying '%Y-%m-%d'")
...
def hidden_code_handler(code_str: str) -> None:
    '''
    Check if are been inserted hidden codes in the textbox 'name'.

    Args:
        - code_str (str): The string writed in the textbox by the user.
